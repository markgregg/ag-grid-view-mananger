      ...
      return viewNames()
        .map(vn => {
          ...
          return viewString != null
            ? JSON.parse(viewString) as View
            : null;
        })
        .filter(v => v != null) as View[];
    },
    getActiveView: () => {
      return window.localStorage.getItem(`${id}:ACTIVE`);
    },
    setActiveView: (view: View) => {
      return window.localStorage.setItem(`${id}:ACTIVE`, view.id);
    },
    persist: (view: View) => {
      window.localStorage.setItem(`${id}:VIEW:${view.id}`, JSON.stringify(view));
      const views = viewNames();
      if (!views.includes(view.id)) {
        window.localStorage.setItem(`${id}:VIEWS`, [...viewNames(), view.id].join(','));
      }
    },
    delete: (view: View) => {
      window.localStorage.removeItem(`${id}:VIEW:${view.id}`);
      window.localStorage.setItem(`${id}:VIEWS`, viewNames().filter(vm => vm !== view.id).join(','));
    },
  };
}